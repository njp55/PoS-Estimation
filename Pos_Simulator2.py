...
def generate_excel_template():
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        pd.DataFrame(proj_export, columns=["ID", "Modality", "Indication", "Current Phase"]).to_excel(writer, sheet_name='Projects', index=False)
        
        if proj_params_export:
            pd.DataFrame(proj_params_export).to_excel(writer, sheet_name='Project_Parameters', index=False)
        else:
            # プロジェクトパラメータが空の場合もシートの枠組みだけは作成
...
with tab1:
    ...
    with st.expander("📥 データのインポート / 📤 エクスポート", expanded=True):
        st.markdown("現在の設定値、およびパイプライン一覧に登録された全てのプロジェクトデータ（個別パラメータの数値含む）をExcelにバックアップしたり、一括で読み込むことができます。")
        ...
        if uploaded_file is not None:
            if st.button("インポートを実行", type="primary"):
                try:
                    ...
                    elif 'Parameters' in xls.sheet_names: # 旧バージョンとの互換性
                    ...
                    if 'Projects' in xls.sheet_names:
                        ...
                        # プロジェクト個別パラメータがあれば読み込む
                        if 'Project_Parameters' in xls.sheet_names:
                            proj_params_df = pd.read_excel(xls, 'Project_Parameters')
                        else:
                            proj_params_df = pd.DataFrame()
                            
                        new_projects = []
                        for _, row in proj_df.iterrows():
                            # 対象プロジェクトの個別パラメータを抽出
                            if not proj_params_df.empty and 'Project_ID' in proj_params_df.columns:
                                my_params = proj_params_df[proj_params_df['Project_ID'] == row['ID']].copy()
                                my_params['Apply'] = True # 復元されたパラメータは適用状態とする
                                my_params = my_params.drop(columns=['Project_ID']) # アプリ内の構造に合わせる
                            else:
